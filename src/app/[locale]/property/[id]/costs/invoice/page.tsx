...
import InvoicePanel from '@/components/costs/InvoicePanel';
...
export default async function InvoicePage({ params: { locale, id } }: Props) {
  const [property, invoices, tracker] = await Promise.all([
    getProperty(id),
    getInvoices(id),
    getTaxTracker(id),
  ]);
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10">
      ...
      <nav className="text-sm text-stone-500 flex items-center gap-1.5 mb-6">
        ...
        <span>/</span>
        <Link
          href={`/${locale}/property/${id}/costs`}
          className="hover:text-stone-900 transition-colors"
        >
          Costs
        </Link>
        <span>/</span>
        <span className="text-stone-900 font-medium">Invoices</span>
      </nav>
