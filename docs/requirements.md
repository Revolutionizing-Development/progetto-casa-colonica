...
| 1.2.30 | 2026-05-16 | **Phasing & Cash Flow table rewrite**: PhaseTimeline year colors consolidated (Y1+Y2=amber, Y3+Y4=emerald). Annual cash flow table restructured: funding detail sub-rows show per-inflow labels with "€X/mo × N" format (from `buildInflowsFromProfile()`), "Cash drawn from savings" row replaces "Net cash needed", "Running cash balance" row tracks cumulative balance from `starting_cash`. Starting cash balance shown as blue header row. Per-year summary cards show `totalOutflow` with purchase+renovation breakdown. Household profile warning when profile is missing. Liquidity warning when running balance goes negative. | User testing: "Gantt bars wrong colors, funding labels missing, starting cash not tracked, row naming confusing" |
| 1.2.31 | 2026-05-16 | **Rendering auto-detects view type from photo classification**: RenderingsPanel no longer asks user to select rendering view type. Photo category (set during upload) auto-maps to rendering type via `PHOTO_TO_RENDERING` lookup (12 categories → 7 rendering types + 2 non-renderable). Read-only detected-type label with optional "Reclassify" override dropdown. Non-renderable photo categories (roof, detail) show warning and disable generate button. Photo thumbnails display category labels. | User testing: "Rendering page redundantly asks for view type already classified during photo upload" |
| 1.2.32 | 2026-05-16 | **Transport hubs and route polylines on Location & Life map**: Transport hub data (train stations + airports) now sourced from AI analysis instead of Mapbox POI search — AI provides connection details (destinations, airlines, travel times) that Mapbox cannot. Coordinates verified via Mapbox Directions API (`refineTransportHub()`). Service POIs (supermarket, bakery, pharmacy, hospital, vet) remain Mapbox-sourced. Map upgraded: transport hub markers (blue=train, indigo=airport) with connection popups, driving route polylines from property to all POIs and hubs (color-coded by type), bounds fit to include all markers. New "Transport Connections" section with train station cards (connection pills) and airport cards (airline/route pills). Map legend updated with route color indicators. `TransportHub` interface added to types. AI prompt expanded to research 1-2 intercity train stations and ALL airports within 2.5h drive. | User testing: "Location tab missing map visualization with transport links" |
